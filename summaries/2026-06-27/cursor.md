# Cursor 每日技术文档 — 2026-06-27

> 本地实测版本：**—（桌面未实测）**｜监测源：[Cursor Changelog](https://cursor.com/changelog)、[Cursor Docs](https://cursor.com/docs)

## 今日综述

**2026 年 6 月 27 日 Cursor 官方 Changelog 无新条目。** 最新版本仍为 **3.9（2026-06-22）**。本周主线：**Customize 页**、**Automations**（`/automate`、cron、computer use）、**Cloud Agent**、**Bugbot `/review`**、**`.cursor/permissions.json`**。本 DayAI 任务即 **3.8 cron 实例**（`0 22 * * *` UTC）。⚠️ **Cloud Agent 无法 GUI 测试** Customize 页、Design Mode、Automations UI、本地 `/review`。
...
| 项 | 结果 |
|----|------|
| 本任务 cron | ✅ 2026-06-27T22:00Z 前后启动 |
...
Cloud Agent 在独立 VM 运行：**environment setup** <10 分钟生成 snapshot；**`/in-cloud`** 启动 cloud subagent；**`/babysit`** 远程迭代 PR；**Handoff** 本地↔云端迁移。⚠️ **Cloud Agent 无法 GUI 测试** Customize、Design Mode、本地 Bugbot。
...
Bugbot 由 **Composer 2.5** 驱动：review ~90 秒、bug 发现 +10%、成本 -22%。**`/review`** push 前运行 Bugbot + Security Review；与 GitHub/GitLab 同步避免重复 review；可配置仅 review 新变更。
...
**`.cursor/permissions.json`** 定义 Agent 操作边界：文件读写、Shell/MCP、自动运行策略。SDK **`local.autoReview`** 配合 `autoRun.allow/block_instructions` 自然语言 steer 分类器。User 级与项目级**数组合并**生效。对 **Computer use + cron Automation** 尤为关键。
...
- Agent / Cloud Agent / SDK 环境；permissions 纳入 PR 评审

### 详细使用步骤（业务用户）

1. 仓库根创建 `.cursor/permissions.json`
2. 定义 `allow` / `deny` / `ask`（路径与 Shell glob）
3. SDK headless：设 `local.autoReview: true` + `autoRun` 指令
4. **Settings → Cursor Settings → Rules and Permissions → Edit permissions.json**
5. 提交 Git，Automation 运行时自动加载
...
| Changelog 6/4 | ✅ autoReview 一致 |

### 问题与解决方案

**越权执行**：检查 deny glob 覆盖。**Cloud/local 不一致**：确认 user+project 合并结果。**autoReview 未拦截**：补充 `block_instructions`。
...
|------|--------|
| Changelog 6/4 SDK | ✅ |
| Docs permissions reference | ✅ allow/deny/ask |
...
| 3.7 | 6/10 | Bugbot Composer 2.5、`/review`、permissions autoReview（6/4 SDK） |

## 今日研究员结论

Cursor **6/27 仍为消化期**：无新 Changelog。本 DayAI 验证 cron + Cloud Agent 闭环；⚠️ Cloud 无法 GUI 测试 Customize、Design Mode、`/review`。

**行动建议**：桌面升级 3.9；Automation 必配 permissions.json；push 前 `/review`；Cloud 用三配置文件替代 GUI。
