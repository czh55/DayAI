...
2026 年 8 月 1 日 Claude Code **无新版本发布**，npm `@latest` 与 Changelog 首条仍为 **2.1.220**（7/25 01:35 UTC，bug fixes and reliability improvements）。自 7/25 起已连续 **8 日**无功能迭代，进入维护冻结第 **8** 日。**Opus 5**（`claude-opus-5`）于 7/24 随 2.1.219 成为默认 Opus 模型，8/1 进入发布第 **9** 日。

行业叙事焦点转向 Boris Cherny YC 访谈余温：随 Opus 5 上线 Claude Code system prompt **精简超 80%**；Boris 提出 Harness **「保质期约半年」**、**Unhobbling（解缚）** 与 **verification loop** 取代堆 prompt（[industry.md](./industry.md)、[china-media.md](./china-media.md)）。DeepSeek V4-Flash API 公测第 2 日（[china-ai.md](./china-ai.md)）；Codex **0.146.0** stable（[codex.md](./codex.md)）；Cursor iPad PR Review 第 4 日（[cursor.md](./cursor.md)）。
...
## 特性一：Opus 5 第 9 日观察——`claude-opus-5`、1M context、`/effort` 与 `/fast`（7/24 起）

### 是什么（机制说明）

Opus 5（`claude-opus-5`）是 Anthropic 7/24 发布的前沿 Opus 模型，随 **2.1.219** 成为默认 Opus 选项；2.1.219 修复 `/model` 显示「Opus (1M context)」标签。核心能力：**1M context** 窗口；**effort 调速**（low/medium/high/xhigh）；**Fast mode**（`/fast`，约 2.5x 速度、2x 价格 $10/$50 per Mtok）；API 标准定价 $5/$25 per Mtok。8/1 第 9 日 CLI 无变更；Boris Cherny 访谈指出 Opus 5 使 Claude Code 可删 80% system prompt，印证「模型越强、Harness 越薄」趋势（详见 [industry.md](./industry.md)）。
...
| 8/1 更新 | ✅ 无 |
| Boris YC 访谈（[china-media.md](./china-media.md)） | ✅ 删 80% prompt |
| DeepSeek V4-Flash（[china-ai.md](./china-ai.md)） | ⚠️ 后训练对照 |
...
2.1.219 新增 **`sandbox.network.strictAllowlist`**：沙箱命令访问非白名单主机**直接拒绝**，不弹权限提示。与 `sandbox.network.allowlist` 配合形成显式允许 + 严格模式。`**--safe-mode**` 启动额外安全层，限制 customizations，适合 CI 与高合规场景。8/1 无变更；可组合 AI 编程栈叙事（[industry.md](./industry.md)）中，Claude Code 执行层 + strictAllowlist 成企业采纳关键门槛。
...
2.1.219 增强 MCP 诊断：`claude mcp list` 与 `/mcp` 连接失败时显示 **HTTP 状态**与**错误文本**；对配置值隐藏首尾空白发警告。headless stream-json init 事件新增 **`mcp_server_errors`**，列出 `--mcp-config` 被跳过条目。`--strict-mcp-config` 仅使用指定 MCP 配置。8/1 无变更；Codex `codex-plugin-cc` 官方插件持续推动与 Claude Code 叠用（[codex.md](./codex.md)），MCP 治理成跨工具共性需求。
...
## 特性五：2.1.220 维护冻结第 8 日 + Boris Harness 哲学——版本策略与「解缚」实践

### 是什么（机制说明）

**2.1.220** 于 7/25 01:35 UTC 发布，Changelog 标注「bug fixes and reliability improvements」，无新功能。8/1 为 npm `@latest` **冻结第 8 日**。2.1.220 承载 2.1.219 全部能力（Opus 5、depth 3、strictAllowlist、MCP 诊断、workflowSizeGuideline medium），属稳定性补丁层。

并行行业叙事：Boris Cherny YC 7/28 访谈——**Harness 保质期约半年**、删 **80% system prompt**、**Unhobbling** 与自我验证循环（[china-media.md](./china-media.md)），与 DeepSeek 后训练叙事形成共识。
...
|----|------|
| `claude --version` | ✅ `2.1.220 (Claude Code)` |
| npm `@latest` / Changelog 8/1 | ✅ 2.1.220，无新条目 |
| 升级路径 | ✅ `npm install -g` 可用 |
| Boris 删 80% prompt | ✅ 媒体报道 + Anthropic 7/24 方向一致（无 CLI 新 flag） |

### 问题与解决方案

**版本不一致**：全局与项目本地可能不同；用 `which claude` 定位。**误升旧版**：企业 managed settings 可能 pin 版本。**长期冻结担忧**：2.1.219→2.1.220 仅 bug fix，功能完整。**不敢删 Harness**：从非关键规则起步；用测试集替代「感觉会坏」的直觉。**半年未审计**：旧 Sonnet 3.5 时代规则在 Opus 5 下可能过度约束。
...
| npm registry / GitHub Releases | ✅ `@latest` = 2.1.220，8/1 无新 tag |
| Boris YC 访谈 / 量子位 | ✅ 删 80% prompt、半年保质期、Unhobbling |
| Anthropic 7/24 上下文工程 | ✅ 精简 prompt 方向一致 |
| 昨日（[../2026-07-31/claude-code.md](../2026-07-31/claude-code.md)） | ✅ 冻结第 7→8 日 |
...
| 版本 | 发布日期 (UTC) | 要点 |
|------|----------------|------|
| **2.1.220** | 2026-07-25 01:35 | Bug fixes and reliability improvements（**当前 `@latest`，冻结第 8 日**） |
...
Claude Code 8/1 处于 **2.1.220 维护冻结第 8 日**，无新版本。Opus 5 第 **9** 日 CLI 无变更，建议巩固：`/model`+`/effort`+`/fast`、depth 3+`/loops`、strictAllowlist+`--safe-mode`、MCP `mcp_server_errors`、版本锁定策略。**今日最大增量不在 Changelog，而在 Boris Harness 哲学**：删 80% prompt、半年保质期、**Unhobbling + verification loop**，与 DeepSeek 后训练叙事（[china-ai.md](./china-ai.md)）形成共识。竞品 Codex 0.146 第 4 日（[codex.md](./codex.md)）、Cursor iPad 第 4 日（[cursor.md](./cursor.md)）推动可组合栈（[industry.md](./industry.md)）。生产保持 2.1.220，关注 2.1.221+。推理级功能 ⚠️ 未实测（无 API Key）。昨日见 [../2026-07-31/claude-code.md](../2026-07-31/claude-code.md)。
